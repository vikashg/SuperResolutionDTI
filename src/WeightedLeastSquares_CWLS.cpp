...
vnl_matrix<double> WeightedLeastSquares::ComputeJacobian(DiffusionTensorType D)
{
	...
	Rho_vec.set_size(6);
	Rho_vec[0]=  P_upper(0,0);  // rho 2
	Rho_vec[1] = P_upper(1,1); // rho 3
	Rho_vec[2] = P_upper(2,2); // rho 4
	Rho_vec[3] = P_upper(0,1); // rho 5
	Rho_vec[4] = P_upper(1,2); // rho 6
	Rho_vec[5] = P_upper(0,2); // rho 7


	vnl_matrix<double> J; 
	J.set_size(6,6); J.fill(0.0);
...
WeightedLeastSquares::VectorialImageTensorType::Pointer  WeightedLeastSquares::ComputeDelSim(TensorImageType::Pointer tensorImage)
{
	 ...
	 if (itHRMask.Get() != 0)
	{
		 ...
		 for(int i=0; i < numOfImages; i++)
		{
			...
			S.set(S_vec);
			delF = J.transpose()*W.transpose()*S*r;
...
WeightedLeastSquares::TensorImageType::Pointer WeightedLeastSquares::MakeTensorImage(VectorialImageTensorType::Pointer vecImage)
{
	
	TensorImageType::Pointer tensorImage = TensorImageType::New();
	CopyImage cpImage;
	cpImage.CopyTensorImage(m_tensorImage_init, tensorImage);

	VecTensorImageIterator itVec(vecImage, vecImage->GetLargestPossibleRegion());
	ScalarImageIterator itMask(m_HRmask, m_HRmask->GetLargestPossibleRegion());
	TensorImageIterator itTens(tensorImage, tensorImage->GetLargestPossibleRegion());

	for (itMask.GoToBegin(), itVec.GoToBegin(), itTens.GoToBegin(); 
		!itMask.IsAtEnd(), !itVec.IsAtEnd(), !itTens.IsAtEnd();
		++itMask, ++itVec, ++itTens)
	{
		if (itMask.Get() != 0)
		{
			DiffusionTensorType D;
			VectorialTensorType vec = itVec.Get();
			D(0,0) = vec[0]*vec[0];
			D(1,1) = vec[1]*vec[1] + vec[3]*vec[3];
			D(2,2) = vec[2]*vec[2] + vec[4]*vec[4] + vec[5]*vec[5];
			D(0,1) = vec[0]*vec[3];
			D(1,2) = vec[1]*vec[4] + vec[3]*vec[5];
			D(0,2) = vec[0]*vec[5];
...
//				std::cout << tempVector << std::endl;
		...
		TensorImageType::Pointer tensorImage_n_1 =  MakeTensorImage(vecTensorImage_n_1);
