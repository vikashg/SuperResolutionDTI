...
JointTensorEstimation::ImageListType JointTensorEstimation::ComputeDifferenceImages_Frac(TensorImageType::Pointer logTensorImage, ImageListType AttenuationList)
{
	...
	for (int i=0; i < numOfImages ; i++)
	{
				...
				!itPredHR.IsAtEnd(), !itHRMask.IsAtEnd(), !itB0.IsAtEnd(), !itTens.IsAtEnd(); ++itPredHR, ++itHRMask, ++itB0, ++itTens)
		{
		  ...
		  RealType atten_i;
		  vnl_vector<RealType> g_i= m_GradList[i]->GetPixel(itHRMask.GetIndex()).GetVnlVector();
		...
		subtractImageFilter->SetInput2(m_DWIList[i]);
		...
		divideImageFilter->SetInput2(m_B0Image_HR);
...
JointTensorEstimation::ImageListType JointTensorEstimation::ComputeDifferenceImages(TensorImageType::Pointer logTensorImage, ImageListType AttenuationList)
{
...
JointTensorEstimation::TensorImageType::Pointer JointTensorEstimation::ComputeDelSim_Frac_DispField(TensorImageType::Pointer logTensorImage)
{
	...
	DiffImageList = ComputeDifferenceImages(logTensorImage, AttenuationList);
...
JointTensorEstimation::TensorImageType::Pointer JointTensorEstimation::UpdateTerms()
{
    ...
    totEnergy.ReadB0Image(m_B0Image_HR);
