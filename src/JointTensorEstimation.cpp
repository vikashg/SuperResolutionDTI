...
JointTensorEstimation::TensorImageType::Pointer JointTensorEstimation::ComputeDelSim_Frac(TensorImageType::Pointer logTensorImage)
{
				...
				++itMask, ++itDelSimTotal, ++itLogTens, ++itB0)
		{
			if (itMask.Get() != 0)
			{
				...
				for (int i=0; i < numOfImages; i++)
				{
					...
					RealType obsValue = m_DWIList[i]->GetPixel(itMask.GetIndex())/itB0.Get();
